...
class TestLog(TestCase):
    ...
    def setUpClass(self):
        ...
        utils.create_folder(self._path_log)

    @patch('helper.log.utils')
    @patch('helper.log.Config')
    def test_setup_log(self, mock_Config, mock_utils):
        
        mock_Config.return_value.get_env = MagicMock(return_value="test")
        mock_Config.return_value.get_job_process_folder = MagicMock(return_value=self._path)

        logger = Log()
        logger.process_name = "job-routes"

        logger.setup_log()
        file_log_is_created = False
        self.assertEqual(logger.logger.name, 'job-routes')
